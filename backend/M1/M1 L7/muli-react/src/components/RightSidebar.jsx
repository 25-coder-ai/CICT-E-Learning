...
function BookPanel({ data }) {
  ...
  const desc = Array.isArray(data.description) ? data.description : [data.description];
  ...
  return (
    <>
      ...
      <div style={{ ...BODY_STYLE, position: 'relative' }}>
        ...
        {showEnglish
          ? <p style={{ lineHeight: 1.8, color: '#000060' }}>{data.englishDescription}</p>
          : desc.map((para, i) => (
              <p key={i} style={{ lineHeight: 1.8, marginBottom: 8 }}>{para}</p>
            ))
        }

        {data.englishLinkLabel && data.englishDescription && (
          <button
            onClick={toggle}
            style={{
              display: 'block',
          ...
          >
            {showEnglish ? 'தமிழ்' : data.englishLinkLabel}
          </button>
        )}
      </div>
    </>
