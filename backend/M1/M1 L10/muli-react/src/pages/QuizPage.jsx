  ...
  return (
    ...
    >
      ...
      <div style={{ background:`linear-gradient(135deg,${BLUE_DARK},${BLUE_MID})`, color:'#fff', borderRadius:'8px 8px 0 0', padding:'8px 14px', fontSize:12, fontWeight:'bold', display:'flex', justifyContent:'space-between', boxShadow:'0 2px 8px rgba(0,20,100,0.2)' }}>
        <span style={{fontFamily:FONT}}>அலகு 1 : பாடம் 10 : ஐங்குறுநூறு 431</span>
...
function DragDropScreen({ q, index, total, onCorrect }) {
  const wordBank = q.givenLines || q.wordBank || [];
  const [pool, setPool]         = useState(() => shuffle([...wordBank]));
  const [matched, setMatched]   = useState({});
  const [wrongSlot, setWrongSlot] = useState(null);
  const dragging = useRef(null);

  const matchedCount = Object.keys(matched).length;
  const allMatched   = matchedCount === q.correctOrder.length;
  ...
  const onDrop = (slotIdx, e) => {
    e.preventDefault();
    if (matched[slotIdx] !== undefined) return;
    const item = dragging.current || e.dataTransfer.getData('text/plain');
    if (!item) return;
    if (item === q.correctOrder[slotIdx]) {
      setMatched(m => ({ ...m, [slotIdx]: item }));
  ...
  return (
    ...
    >
      ...
      <div style={{ background:`linear-gradient(135deg,${BLUE_DARK},${BLUE_MID})`, color:'#fff', borderRadius:'8px 8px 0 0', padding:'8px 14px', fontSize:12, fontWeight:'bold', display:'flex', justifyContent:'space-between', boxShadow:'0 2px 8px rgba(0,20,100,0.2)' }}>
        <span style={{ fontFamily:FONT }}>வரிசைப்படுத்துக</span>
      ...
      <div style={{ textAlign:'center', marginBottom:10 }}>
        <span style={{ background:'#e8f0ff', border:`1px solid ${BLUE_MID}`, borderRadius:20, padding:'3px 14px', fontSize:11.5, color:BLUE_DARK, fontWeight:'bold' }}>
          {matchedCount} / {q.correctOrder.length} வரிசைப்படுத்தப்பட்டது
        </span>
      </div>

      {/* Drop slots */}
      <div style={{ display:'flex', flexDirection:'column', gap:8, marginBottom:14 }}>
        {q.correctOrder.map((_, slotIdx) => {
          const isMatched = matched[slotIdx] !== undefined;
          const isWrong   = wrongSlot === slotIdx;
          return (
            <motion.div key={slotIdx}
              initial={{ opacity:0, x:-10 }} animate={{ opacity:1, x:0 }} transition={{ delay:0.05*slotIdx+0.1 }}
              style={{ display:'flex', alignItems:'center', gap:8 }}
            >
              <span style={{ fontSize:13, color:'#94a3b8', minWidth:22, fontWeight:'bold', flexShrink:0 }}>{slotIdx+1}.</span>
              <motion.div
        ...
        {pool.length > 0 && (
          <motion.div initial={{ opacity:0, y:8 }} animate={{ opacity:1, y:0 }} exit={{ opacity:0 }}>
            <p style={{ fontSize:11.5, color:'#64748b', marginBottom:6, fontStyle:'italic' }}>
              ✦ அடிகளை இழுத்து வரிசையில் வைக்கவும்
            </p>
      ...
      <AnimatePresence>
        {allMatched && (
          ...
          >
            ...
            <div>
              <div style={{ fontWeight:'bold', fontSize:14, color:'#14532d', fontFamily:FONT }}>சரியாக வரிசைப்படுத்தினீர்கள்!</div>
              <div style={{ fontSize:12, color:'#166534', fontFamily:FONT }}>அடுத்த வினாவிற்கு செல்கிறோம்…</div>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
...
export default function QuizPage() {
  ...
  const scorableCount = questions.filter(q => q.type==='mcq'||q.type==='word-order'||q.type==='drag-drop'||q.type==='drag-and-drop'||q.type==='match-following'||!q.type).length;
  ...
  const renderQuestion = (q, idx) => {
    const scorableIdx = questions.slice(0,idx+1).filter(x => x.type==='mcq'||x.type==='word-order'||x.type==='drag-drop'||x.type==='drag-and-drop'||x.type==='match-following'||!x.type).length - 1;
    ...
    if (q.type==='drag-drop'||q.type==='drag-and-drop') return <DragDropScreen q={q} index={scorableIdx} total={scorableCount} onCorrect={handleCorrect} />;
