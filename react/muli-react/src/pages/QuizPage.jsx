...
───────────────────────────────────────────*/
function getOptionStyle(submitted, isSel, isCorrect) {
  if (!submitted) {
    return {
  ...
  if (isCorrect)        return { bg:'#dcfce7', border:'2px solid #22c55e', color:'#14532d', shadow:'0 0 0 3px rgba(34,197,94,0.2)' };
  ...
  return (
    ...
    >
      ...
      <div style={{ background:`linear-gradient(135deg,${BLUE_DARK},${BLUE_MID})`, color:'#fff', borderRadius:'8px 8px 0 0', padding:'8px 14px', fontSize:12, fontWeight:'bold', display:'flex', justifyContent:'space-between', boxShadow:'0 2px 8px rgba(0,20,100,0.2)' }}>
        <span style={{fontFamily:FONT}}>அலகு 1 : பாடம் 1 : பாடல் 3</span>
      ...
      {q.question && (
        <motion.p initial={{opacity:0,x:-8}} animate={{opacity:1,x:0}} transition={{delay:0.15}}
          style={{ fontSize:14, fontWeight:'bold', color:'#b91c1c', marginBottom:10, paddingLeft:4, lineHeight:1.6 }}
        >{q.question}</motion.p>
      )}

      {/* Options */}
      <div style={{ display:'flex', flexDirection:'column', gap:9, marginBottom:10, flex:1 }}>
        {q.options.map((opt, oi) => {
          const isSel     = selected === opt.id;
          const isCorrect = opt.correct;
          const s         = getOptionStyle(submitted, isSel, isCorrect);
          ...
          return (
            <motion.label key={opt.id}
              ...
              transition={
                ...
                : submitted && isCorrect ? { duration:0.5, delay:0.05 }
                : { delay:0.08*oi+0.1 }
              }
            ...
            >
              ...
              <AnimatePresence>
                {submitted && (isCorrect || (isSel && !isCorrect)) && (
        ...
        {q.options.map((opt, oi) => {
          ...
          return (
            <motion.div key={opt.id}
              ...
              transition={
                submitted && isSel && !isCorrect ? { duration:0.5 }
                : submitted && isCorrect ? { duration:0.55, delay:0.05 }
                : { delay:0.1*oi+0.1 }
              }
...
───────────────────────────────────────────*/
function ReadAloudScreen({ q, onNext }) {
  return (
    <motion.div initial={{opacity:0,y:18}} animate={{opacity:1,y:0}} transition={{duration:0.4}}
      style={{ padding:'14px 18px', fontFamily:FONT, overflowY:'auto', flex:1, display:'flex', flexDirection:'column' }}
    >
